...
- Sản phẩm: 27 Business Products (sách PDF + frameworks + templates)
...
- Live URL: https://foundry-jade.vercel.app/v7 (phiên bản mới nhất)
...
=== CÁC PHIÊN BẢN ===
- / (index.html) = v2 gốc
- /v3, /v4, /v5, /v6 = các iteration cũ, KHÔNG sửa
- /v7 = PHIÊN BẢN ĐANG LÀM ← tập trung vào đây

=== FILE CHÍNH ===
v7/index.html — chỉnh sửa file này
...
=== STRUCTURE v7 (thứ tự sections) ===
Nav → Hero (fan 7 bìa sách mới + CTA) → Proof Bar (27 products, 88%) → Problem →
Outcomes (2-col grid) → 6 Tracks (dark) → The Library (27-book grid + filter tabs) →
Why FOUNDRY (2-col: text + web-mockup-2) → Transformation (dark, card ngang) →
...
- 27 bìa sách PNG mới: press-01-profit-levers.png ... press-27-shockproof-finance.png
- 4 ảnh web mockup: web-mockup-1.png (pricing), web-mockup-2.png (why-foundry), web-mockup-3/4.png (dự phòng)
- Bìa cũ (v2–v6): *.webp (giữ nguyên để v6 không hỏng)

=== 27 SÁCH (v7) — theo track ===
Strategy: Profit Levers, Profit Logic, Growth Engine Architecture, The Advantage Map, The Intelligent Enterprise Engine, Algorithms to Advantage
Finance: Shockproof Finance, The Revenue Shape System, Failure-Proof Operator
Leadership: Stay Power, The Talent Continuity System, The Workplace Energy System, From Resistance to Rollout
Marketing: Brand Gravity, Audience Magnet Lab, Pathways to Purchase, From Click to Commitment, The Relationship Revenue System, The Content Engine Room
Operations: Sprint Engine, Focus Lock, Momentum Stack, Signal Mirror
Innovation: The Advisory Method, The Consultant's Operating System, The Online Store Operating System, The Everyday Security System

=== QUY TẮC QUAN TRỌNG ===
1. Chỉ sửa v7/index.html trừ khi được yêu cầu rõ ràng
2. Không sửa vercel.json, index.html (v2), v3/, v4/, v5/, v6/
3. Sửa HTML bằng editor — KHÔNG dùng PowerShell Set-Content (hỏng UTF-8)
4. Sau mỗi thay đổi: git add v7/index.html && git commit -m "..." && git push
5. Verify: curl -s -o /dev/null -w "%{http_code}" https://foundry-jade.vercel.app/v7 → phải 200
...
🔴 2. Upload 27 PDF thật lên Gumroad product
🟠 3. Thay testimonials placeholder bằng review thật
🟡 4. Verify GA4 realtime hoạt động (vào GA → Báo cáo → Thời gian thực)
🟡 5. Kết nối PayPal vào Gumroad (Gumroad Settings → Payments)
